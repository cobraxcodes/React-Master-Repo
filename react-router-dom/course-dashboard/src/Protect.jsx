...
function Protect ({userLogin, setUserLogin, children}){
    if(!userLogin){
        return <Login setUserLogin={setUserLogin} /> // if not logged in render the login page with the setUserLogin so that it can be set to true or false depending if the user provides correct credentials
    }else{
